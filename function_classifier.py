...
def get_hand(ranks, flush, freq, dist):
    hand = 0
    if freq == [2,1,1,1]:
        hand = 1
    elif freq == [2,2,1]:
        ...
        if dist == 5:
            hand = (4 if flush == False else 8)
            return hand
        elif ranks.tolist() == [1,10,11,12,13]:
            hand = (4 if flush == False else 9)
            return hand
    if flush:
        hand = 5
    if freq == [3,2]:
...
def poker_hand_classifier(data):
...
if __name__ == "__main__":
    data = input("Enter 10 numbers in the format of Suit Card 1, Rank Card 1, Suit Card 2, Rank Card 2, etc. for 5 cards in total. " +
        "Each number should be separated by commas. Aces are 1, Kings are 13: ")
    ...
    print(f"Your hand is a {hands[poker_hand_classifier(data)]}!")
